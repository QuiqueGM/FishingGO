...
public class HBAO_Integrated : HBAO_Core
{
    ...
    void OnPreRender()
    {
        ...
        bool shouldPrepareCommandBuffer = false;
            ...
            _colorBleedingEnabled != colorBleedingSettings.enabled || _blurAmount != blurSettings.amount)
        {
            ...
            shouldPrepareCommandBuffer = true;
            //Debug.Log("Should prepare command buffer!");
        }

        var cameraEvent = GetCameraEvent();
        if (shouldPrepareCommandBuffer || _hbaoCamera.GetCommandBuffers(cameraEvent).Where(x => x.name == "HBAO").Count() == 0)
        {
            ClearCommandBuffer();
