...
public class SchoolChild_BCK: MonoBehaviour{
    public bool hasAnimation = true;
	[HideInInspector]
	public SchoolController_BCK _spawner;
	Vector3 _wayPoint;
	[HideInInspector]
	...
	public bool Avoidance() {
		...
		//Up / Down avoidance
		if (Physics.Raycast(_cacheTransform.position, -Vector3.up+(cacheForward*.01f), out hit, _spawner._avoidDistance, _spawner._avoidanceMask)){			
		...
		if (Physics.Raycast(_cacheTransform.position, cacheForward+(cacheRight*Random.Range(-.01f, .01f)), out hit, _spawner._stopDistance, _spawner._avoidanceMask)){		
	//					Debug.DrawLine(_cacheTransform.position,hit.point);
