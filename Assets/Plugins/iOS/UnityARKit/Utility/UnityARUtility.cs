...
{
	public delegate void SurfaceDetectedRequest();

	public class UnityARUtility
	{
		public static event SurfaceDetectedRequest SurfaceDetectedEvent;
		...
		private static GameObject parentPrefab = null;

		public static void InitializePlanePrefab(GameObject go, GameObject parent)
		{
			planePrefab = go;
			parentPrefab = parent;
		}
		...
		public static GameObject CreatePlaneInScene(ARPlaneAnchor arPlaneAnchor)
		{
			GameObject plane;
			if (planePrefab != null) {
				plane = GameObject.Instantiate(planePrefab, parentPrefab.transform);
				SurfaceDetectedEvent ();
			} else {
